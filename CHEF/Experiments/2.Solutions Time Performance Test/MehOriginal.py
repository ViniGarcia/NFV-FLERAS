...
class Exhaustive:
	...
	def __mEvaluate(self):
		...
		compare = self.__mPartialResults[0]['TR']

		for index in range(1, len(self.__mPartialResults)):

			if self.__mPartialResults[index]['TR'] < compare:
				self.__mSuggestion = [index]
				compare = self.__mPartialResults[index]['TR']
				continue
...
class Heuristic:
	...
	def __mExpand(self, possibilities):
		compare = self.__mElements[possibilities[0]]['TR']
		chosen = 0

		for index in range(1, len(possibilities)):
			if self.__mElements[possibilities[index]]['TR'] < compare:
				chosen = index
...
#eComposer.cCompose("MehOriginal.yaml")
#print(eComposer.cFirmSuggestion())
#hComposer = Composer(2)
#hComposer.cCompose("MehOriginal.yaml")
