...
def timing(rep, file, confs, exhaustive):
    ...
    if exhaustive:
        ...
        for test in range(rep):
            start = time.time()
            subprocess.check_call("ExhaustiveMapping.py " + file, shell=True, stdout= subprocess.PIPE, stderr= subprocess.PIPE)
    ...
    for execution in confs:
        ...
        for test in range(rep):
            start = time.time()
            subprocess.check_call("GeneticMapping.py " + file + " " + execution, shell=True, stdout= subprocess.PIPE, stderr= subprocess.PIPE)
            timeResults.append(time.time() - start)
...
def quality(rep, file, confs, mode):
    ...
    subprocess.check_call("ExhaustiveMapping.py " + file + " -o parameter.csv " + mode, shell=True, stdout= subprocess.PIPE, stderr= subprocess.PIPE)
    ...
    for execution in confs:
        ...
        for test in range(rep):
            analyzerFronts = []
            subprocess.check_call("GeneticMapping.py " + file + " " + execution + " -o output.csv", shell=True, stdout= subprocess.PIPE, stderr= subprocess.PIPE)
