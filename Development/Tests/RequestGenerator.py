	...
	def __defineService(self, service, bounds, stype):
		...
		for function in service:
			...
			for resource in bounds:
				if bounds[resource]["BEGIN"] != bounds[resource]["END"]:
					skeleton["SERVICE"]["FUNCTION"][function][resource] = random.randrange(bounds[resource]["BEGIN"], bounds[resource]["END"])
				else: 
					skeleton["SERVICE"]["FUNCTION"][function][resource] = bounds[resource]["BEGIN"]
	...
	def __defineMetrics(self, graph):
		...
		for e_node in graph:
			...
			for resource in self.__metrics["RESOURCE"]:
				if self.__metrics["RESOURCE"][resource]["BEGIN"] != self.__metrics["RESOURCE"][resource]["END"]:
					...
					l_skeleton["RESOURCE"][resource] = self.__metrics["RESOURCE"][resource]["BEGIN"]
			...
			for metric in self.__metrics["LOCAL"]:
				if self.__metrics["LOCAL"][metric]["TYPE"] == "INT":
					if self.__metrics["LOCAL"][metric]["BEGIN"] != self.__metrics["LOCAL"][metric]["END"]:
						...
						l_skeleton["LOCAL"][metric] = self.__metrics["LOCAL"][metric]["BEGIN"]
				else:
				...
				for metric in self.__metrics["TRANSITION"]:
					if self.__metrics["TRANSITION"][metric]["TYPE"] == "INT":
						if self.__metrics["TRANSITION"][metric]["BEGIN"] != self.__metrics["TRANSITION"][metric]["END"]:
							...
							t_skeleton[i_node][metric] = self.__metrics["TRANSITION"][metric]["BEGIN"]
					else:
