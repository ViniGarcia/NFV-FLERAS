...
class ServiceMapping():
	...
	def execute(self, sk, execution, mode):
		...
		while self.__progression(execution, mode, step):
			startTime = time.time()
			current = []
			found = True
...
class Mapping:
	...
	def __lightweightParetoFrontiers(self, results):
		#WARNING: Currently works only for maximization (I will change that later, for hybrid analysis use __paretoFrontier)
		results = numpy.array(results)
		paretoAnlysis = numpy.ones(results.shape[0], dtype = bool)
		for index, value in enumerate(results):
			if paretoAnlysis[index]:
				paretoAnlysis[paretoAnlysis] = numpy.any(results[paretoAnlysis]>value, axis=1)
				paretoAnlysis[index] = True

		return list(numpy.where(paretoAnlysis == True)[0])
	...
	def execute(self, sk, execution, mode):
		...
		evaluations = self.__algorithm.execute(sk, execution, mode)
		return evaluations
	...
	def outputFrontiers(self, filename, result):
		...
		indexFrontiers = self.__lightweightParetoFrontiers(ndResult[1])
		...
		duplicates = []
		for index in indexFrontiers:
			if ndResult[0][index] in duplicates:
				continue
			...
			file.write("0")
			file.write("\n")
			duplicates.append(ndResult[0][index])
...
if len(sys.argv) < 6 or len(sys.argv) > 8:
	...
	print("-> *.py file_name -sk k_value (-r execution_rounds | -t time_secs) [-o output_name]")
	print("========================================================================")
	exit()
...
if len(sys.argv) >= 6:
	...
	if sys.argv[2] == "-sk":
		try:
			...
			print("ERROR: -sk MUST BE AN INTEGER NUMBER!")
			exit()
	else:
		print("ERROR: ESSENTIAL FLAGS ARE NOT DEFINED (-sk)!")
		exit()
	if sys.argv[4] == "-r":
		try:
			n = int(sys.argv[5])
			...
			print("ERROR: -r MUST BE AN INTEGER NUMBER!")
			exit()
	elif sys.argv[4] == "-t":
		try:
			n = int(sys.argv[5])
			...
			print("ERROR: -t MUST BE AN INTEGER NUMBER!")
			exit()
	else:
