...
class FeedbackMechanismGenerator(Generator):
    __search = None
    __constraints = None
    __size = None
    __feedback = None

    def __init__(self, search, constraints, feedback):
        super(FeedbackMechanismGenerator, self).__init__()
    ...
    def generate(self, problem):
        ...
        if len(self.__feedback) == 0:
            ...
            genome = self.__feedback.pop()
            for index in range(len(genome)):
                genome[index] = translator.encode(genome[index])
...
class BitSwap(Mutation):
    ...
    def __init__(self, probability=1):
        """Bit Swap Mutation for Binary Strings.
