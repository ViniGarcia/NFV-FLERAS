...
class Heuristic:
	...
	def __hEvaluate(self, request, function, nodes):
		...
		return min([candidate for candidate in partialResults.items() if candidate[1][self.__hMetric] == min(partialResults.items(), key=lambda x: x[1][self.__hMetric])[1][self.__hMetric]])[0]
...
#mijumbi = Processor()
#mijumbi.pProcess("MijOriginal.yaml")
